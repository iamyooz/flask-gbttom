...
app = Flask(__name__)
CORS(app, supports_credentials=True, origins=["localhost:3000", "https://web-react-wow-54ouz2lllulbggn.sel3.cloudtype.app/"])
...
def load_scaler_and_data():
    ...
    if scaler is None:
        with open('girlish_bottom_cluster.json', 'r') as json_file:  # Changed from casual_top_cluster.json
...
@app.route('/gbottom', methods=['POST'])
@cross_origin()
